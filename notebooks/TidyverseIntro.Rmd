...
start_year <- gdp_df |>
  ...
  pull(min_year)
paste('The first year in the dataset is',start_year)

end_year <- gdp_df |>
  summarize(max_year = max(Year)) |>
  pull(max_year)
paste('The last year in the dataset is',end_year)
```
...
paste('There are',total_countries,'represented in the GDP dataset.')
```
...
neg_change <- gdp_pivoted |>
  summarize(negative_pct_change_count = sum(Percent_Change<0), pct_neg_pct_change = 100*mean(Percent_Change<0))

paste('There were',neg_change$negative_pct_change_count,'countries that experienced a negative percent change in GDP per capita from 1990 to 2021, representing',round(neg_change$pct_neg_pct_change,digits=2),'% of countries.')
```
...
paste(top_2_gdp_increase[1],'had the highest percent increase in GDP per capita from 1990 to 2021.')
paste(top_2_gdp_increase[2],'had the second highest percent increase in GDP per capita from 1990 to 2021.')
...
paste('Countries with the top 3 largest GDPs in 2021:')
top_3_gdp_2021
```
...
le_gdp_cor <- gdp_le_2021 |>
  summarize(le_gdp_cor=cor(GDP_Per_Capita, Life_Expectancy, use='complete.obs'))
...
le_log_gdp_cor <- gdp_le_2021 |>
  summarize(le_log_gdp_cor=cor(Life_Expectancy, Log_GDP_Per_Capita, use='complete.obs'))
